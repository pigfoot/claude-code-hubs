...
## Language Detection & Communication
- **Auto-detect user's primary language** from their messages and conversation history
- Ignore common short phrases (e.g., "ok", "go", "sure", "no problem") when detecting language preference
- If user primarily communicates in non-English:
  - Respond and communicate in their detected language (e.g., Traditional Chinese, Japanese, etc.)
- If user primarily communicates in English or language cannot be determined:
  - Default to English for communication
- **Code, comments, documentation, and README files MUST always be in English** regardless of communication language

## Language Priority Detection Rules
1. Analyze substantial messages (10+ characters or complete sentences)
2. Look for patterns in user's message history
3. Common cross-language phrases don't count: "ok", "go", "yes", "no", "sure", "thanks", "please", etc.
4. Once language preference is detected, maintain consistency unless user switches language
...
4. **Refactor**: Clean up
5. **Commit**: Use `git-commit`skill or say "commit changes"
