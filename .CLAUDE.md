...
**Rules when writing HANDOFF.md:**

- **Reference, don't duplicate** - point to PRDs/plans/reports/commits by path
  instead of copying their content in
- **Redact secrets** - never write API keys, passwords, tokens, or PII into it
- **Check `.gitignore`** - if the project is a git repo, confirm `.claude/`
  (or `HANDOFF.md` specifically) is ignored before writing, since it may
  contain internal paths or context not meant for commit history
