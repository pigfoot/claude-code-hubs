...
General-purpose file attachment upload to Confluence pages with correct
ADF media node referencing (`fileId` / `collectionName`) and optional
inline display (mediaSingle/mediaGroup).
...
- **THEN** the function SHALL fall back to querying
  `GET /wiki/api/v2/attachments/{attachmentId}` to obtain the `fileId`
