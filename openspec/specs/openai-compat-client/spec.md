# openai-compat-client Specification

## Purpose

Defines how `generate_images.py` initializes and uses the OpenAI-compatible client for all image generation
API calls. Introduced by the nano-banana-openai-migration change, replacing the `google-genai` library.

## Requirements

### Requirement: OpenAI-Compatible Client Initialization

The system SHALL initialize an OpenAI client using `IMAGE_GEN_BASE_URL` and `RDSEC_API_KEY`,
and SHALL use this client for all image generation.

**ID:** `openai-compat-client-001`
**Priority:** High
...
The system SHALL use `client.chat.completions.create()` with `extra_body={"response_modalities": ["IMAGE"]}`
for all image generation, including both standard slides and reference-image slides.

**ID:** `openai-compat-client-002`
**Priority:** High
...
- **THEN** `chat.completions.create()` is called with a multi-part message containing the logo image
  as `image_url` with base64 data URI, followed by the text prompt
- **AND** `extra_body={"response_modalities": ["IMAGE"]}` is included
...
The system SHALL extract generated image bytes from `message.images[0].image_url.url`
(accessed via `model_dump()`), with fallback to the `content` list.

**ID:** `openai-compat-client-004`
**Priority:** High
